% pop_std_clustinfo() - sumarize relation of Cluster Vs Subject/Dataset Vs IC
%
% Usage:
%   >>  clustinfo = pop_std_clustinfo(STUDY, ALLEEG);
%
% Inputs:
...
function [STUDY, com] = pop_std_clustinfo(STUDY, ALLEEG, varargin)
...
if nargin < 2
    help pop_std_clustinfo;
    return;
end
if isempty(STUDY) || isempty(ALLEEG)
    fprintf(2,'pop_std_clustinfo error : Invalid inputs. STUDY or ALLEEG are empty\n');
    return;
end
     
if isempty(unique([STUDY.cluster.parent])) || isempty({STUDY.cluster.child})
      fprintf(2,'pop_std_clustinfo : No clusters to display\n')
     return;
 end
...
if ishandle(handles.mainfig)
    ...
    %com            = ['[STUDY , clust_statout] = std_clustinfo(STUDY,ALLEEG,' args{:}  ');']
...
%     display('pop_std_clustinfo: Figure already open');
%     flag_run = 0;
% else
    ...
    [STUDY , clust_statout] = std_clustinfo(STUDY,ALLEEG,args{:});
% end
...
[trash, opts] = propertiesGUI(opts);
warning('on','MATLAB:HandleGraphics:ObsoletedProperty:JavaFrame');
setappdata(handles,'opts',opts);
